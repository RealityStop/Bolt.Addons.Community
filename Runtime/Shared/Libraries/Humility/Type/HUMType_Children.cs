    ...
    public static partial class HUMType_Children
    {
        ...
        public static string Code(this HUMValue.Data.As @as, bool isNew, bool isLiteral = false, bool highlight = true, string parameters = "", bool newLineLiteral = false, bool fullName = false, bool variableForObjects = true)
        {
            ...
            if (type == typeof(Gradient))
            {
                ...
                var colorKeys = value.colorKeys.Select(k =>
                    Create("GradientColorKey",
                        k.color.As().Code(false, false, false, "", false, fullName),
                        k.time.As().Code(false, false, false, "", false, fullName)
                    )
                ).ToArray();

                var alphaKeys = value.alphaKeys.Select(k =>
                    Create("GradientAlphaKey",
                        k.alpha.As().Code(false, false, false, "", false, fullName),
                        k.time.As().Code(false, false, false, "", false, fullName)
                    )
                ).ToArray();

                return
                    $"new Gradient() {"\n" + CodeBuilder.GetCurrentIndent()}{{{"\n" + CodeBuilder.GetCurrentIndent(1)}" +
                    "colorKeys = new GradientColorKey[] { " + string.Join(", ", colorKeys) + $" }},{"\n" + CodeBuilder.GetCurrentIndent()}" +
                    "alphaKeys = new GradientAlphaKey[] { " + string.Join(", ", alphaKeys) + $" }},{"\n" + CodeBuilder.GetCurrentIndent()}" +
                    "mode = " + value.mode.As().Code(false, false, false, "", false, fullName) + "\n" + CodeBuilder.GetCurrentIndent(-1) +
                    "}";
            }
        ...
        private static string HighlightedCode(this HUMValue.Data.As @as, bool isNew, bool isLiteral = false, string parameters = "", bool newLineLiteral = false, bool fullName = false, bool variableForObjects = true)
        {
            ...
            if (type == typeof(Gradient))
            {
                ...
                var colorKeys = value.colorKeys.Select(k =>
                    CreateHighlighted("GradientColorKey",
                        k.color.As().Code(false, false, true, "", false, fullName),
                        k.time.As().Code(false, false, true, "", false, fullName)
                    )
                ).ToArray();

                var alphaKeys = value.alphaKeys.Select(k =>
                    CreateHighlighted("GradientAlphaKey",
                        k.alpha.As().Code(false, false, true, "", false, fullName),
                        k.time.As().Code(false, false, true, "", false, fullName)
                    )
                ).ToArray();

                return
                    "new ".ConstructHighlight() + "Gradient".TypeHighlight() +
                    $"() {(newLineLiteral ? "\n" + CodeBuilder.GetCurrentIndent() : "")}{{{(newLineLiteral ? "\n" + CodeBuilder.GetCurrentIndent(1) : "")}" +
                    $"{"colorKeys".VariableHighlight()} = new ".ConstructHighlight() + "GradientColorKey".TypeHighlight() + "[] { " + string.Join(", ", colorKeys) + $" }},{(newLineLiteral ? "\n" + CodeBuilder.GetCurrentIndent() : "")}" +
                    $"{"alphaKeys".VariableHighlight()} = new ".ConstructHighlight() + "GradientAlphaKey".TypeHighlight() + "[] { " + string.Join(", ", alphaKeys) + $" }},{(newLineLiteral ? "\n" + CodeBuilder.GetCurrentIndent() : "")}" +
                    $"{"mode".VariableHighlight()} = " + value.mode.As().Code(false, false, true, "", false, fullName) + (newLineLiteral ? "\n" + CodeBuilder.GetCurrentIndent(-1) : "") +
                    "}";
            }
            if (type == typeof(WaitForFlowLogic))
            {
        ...
        private static string HighlightedCode(this HUMValue.Data.As @as, bool isNew, bool isLiteral = false, string parameters = "", bool newLineLiteral = false, Func<Type, bool> fullName = null, bool variableForObjects = true)
        {
            ...
            if (type == typeof(Gradient))
            {
                ...
                return
                    "new ".ConstructHighlight() + "Gradient".TypeHighlight() +
                    $"() {"\n" + CodeBuilder.GetCurrentIndent()}{{{"\n" + CodeBuilder.GetCurrentIndent(1)}" +
                    $"{"colorKeys".VariableHighlight()} = " + "new ".ConstructHighlight() + "GradientColorKey".TypeHighlight() + "[] { " + string.Join(", ", colorKeys) + $" }},{"\n" + CodeBuilder.GetCurrentIndent()}" +
                    $"{"alphaKeys".VariableHighlight()} = " + "new ".ConstructHighlight() + "GradientAlphaKey".TypeHighlight() + "[] { " + string.Join(", ", alphaKeys) + $" }},{"\n" + CodeBuilder.GetCurrentIndent()}" +
                    $"{"mode".VariableHighlight()} = " + value.mode.As().Code(false, false, true, "", false, fullName) + "\n" + CodeBuilder.GetCurrentIndent(-1) +
                    "}";
            }
            if (type == typeof(WaitForFlowLogic))
            {
