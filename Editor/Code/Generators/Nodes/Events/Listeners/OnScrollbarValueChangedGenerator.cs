...
{
    [NodeGenerator(typeof(OnScrollbarValueChanged))]
    public class OnScrollbarValueChangedGenerator : EventListenerMethodGenerator<OnScrollbarValueChanged>
    {
        public OnScrollbarValueChangedGenerator(Unit unit) : base(unit) { }
        ...
        {
            return $".GetComponent<{"Scrollbar".TypeHighlight()}>()?.{"onValueChanged".VariableHighlight()}?.AddListener({(!Unit.coroutine ? Name : $"({"value".VariableHighlight()}) => StartCoroutine({Name}({"value".VariableHighlight()}))")});";
        }
