...
namespace Unity.VisualScripting.Community
{
    ...
    {
        public VariableNameInspector(Metadata metadata, Func<IEnumerable<string>> getSuggestions, Action<string, string> onChanged, string controlName) : base(metadata)
        {
        ...
        public Action<string, string> onChanged { get; }
        ...
        protected override void OnGUI(Rect position, GUIContent label)
        {
            ...
            var oldvalue = (string)metadata.value;
            var newValue = EditorGUI.TextField(textFieldPosition, oldvalue, Styles.textField);

            CalculateVariableNameSuggestions(position);

            EditorGUI.BeginDisabledGroup(suggestions.Length == 0);

            var currentSuggestionIndex = Array.IndexOf(suggestions, oldvalue);
