...
namespace Unity.VisualScripting.Community
{
    public class UnitWidget<TUnit> : NodeWidget<FlowCanvas, TUnit>, IUnitWidget where TUnit : class, IUnit
    {
        ...
        private readonly List<(float, SeparatorPosition)> rowDividerPositions = new List<(float, SeparatorPosition)>();

        private readonly List<float> controlInputDividersX = new List<float>();
        private readonly List<float> controlOutputDividersX = new List<float>();

        private readonly List<IUnitPortWidget> _valueInputs = new List<IUnitPortWidget>();
        private readonly List<IUnitPortWidget> _valueOutputs = new List<IUnitPortWidget>();
        private readonly List<ControlInputWidget> _controlInputs = new List<ControlInputWidget>();
        private readonly List<ControlOutputWidget> _controlOutputs = new List<ControlOutputWidget>();

        private enum SeparatorPosition
        {
            Left,
            Right,
            Full
        ...
        public override void CachePosition()
        {
            ...
            _valueInputs.Clear();
            _valueOutputs.Clear();
            _controlInputs.Clear();
            _controlOutputs.Clear();

            float valueInputsWidth = 0f;
            float maxCiWidth = 0f;

            for (int i = 0; i < inputs.Count; i++)
            {
                var p = inputs[i];
                if (p is ValueInputWidget || p is InvalidInputWidget)
                {
                    _valueInputs.Add(p);
                    valueInputsWidth = Mathf.Max(valueInputsWidth, p.GetInnerWidth());
                }
                else if (p is ControlInputWidget ci)
                {
                    _controlInputs.Add(ci);
                    maxCiWidth = Mathf.Max(maxCiWidth, ci.GetInnerWidth());
                }
            }

            float valueOutputsWidth = 0f;
            float maxCoWidth = 0f;

            for (int i = 0; i < outputs.Count; i++)
            {
                var p = outputs[i];
                if (p is ValueOutputWidget || p is InvalidOutputWidget)
                {
                    _valueOutputs.Add(p);
                    valueOutputsWidth = Mathf.Max(valueOutputsWidth, p.GetInnerWidth());
                }
                else if (p is ControlOutputWidget co)
                {
                    _controlOutputs.Add(co);
                    maxCoWidth = Mathf.Max(maxCoWidth, co.GetInnerWidth());
                }
            }

            var portsWidth = valueInputsWidth + Styles.spaceBetweenInputsAndOutputs + valueOutputsWidth;

            if (_controlInputs.Count > 0)
            {
                portsWidth = Mathf.Max(portsWidth, Mathf.Min(90f, maxCiWidth + spaceBetweenControlPorts) * _controlInputs.Count);
            }

            if (_controlOutputs.Count > 0)
            {
                portsWidth = Mathf.Max(portsWidth, Mathf.Min(90f, maxCoWidth + spaceBetweenControlPorts) * _controlOutputs.Count);
            }
            ...
            if (showSettings)
            {
                for (int i = 0; i < settings.Count; i++)
                {
                    ...
                    var content = settingLabelsContents[setting];
                    float settingWidth = 0f;

                    if (content != null)
                        settingWidth += Styles.settingLabel.CalcSize(content).x;
            ...
            var headerTextWidth = Styles.title.CalcSize(titleContent).x;

            if (showSurtitle) headerTextWidth = Mathf.Max(headerTextWidth, Styles.surtitle.CalcSize(surtitleContent).x);
            if (showSubtitle) headerTextWidth = Mathf.Max(headerTextWidth, Styles.subtitle.CalcSize(subtitleContent).x);
            ...
            var headerWidth = Mathf.Max(headerTextWidth + iconsWidth, Mathf.Max(settingsWidth, headerAddonWidth)) + Styles.iconSize + (Styles.spaceAfterIcon * compactX);
            ...
            float ciHeight = 0;
            for (int i = 0; i < _controlInputs.Count; i++)
            {
                if (_controlInputs[i].showLabel)
                {
                    ciHeight = Mathf.Max(ciHeight, _controlInputs[i].GetHeight());
                }
            }

#if !NEW_UNIT_STYLE
            const int NormalStyleExtraHeight = 4;
            if (ciHeight > 0) ciHeight += NormalStyleExtraHeight;
#endif
            headerHeight += ciHeight;
            y += ciHeight;
            ...
            if (showSurtitle)
            {
                ...
                float step = h + Styles.spaceAfterSurtitle * compactY;
                headerHeight += step;
                y += step;
            }
            ...
            if (showTitle)
            {
                if (!showSurtitle && !showSettings && (!showSettings || settings?.Count == 0) && (!showHeaderAddon || GetHeaderAddonHeight(headerAddonWidth) == 0))
                {
                    // Ensure the Title and Unit Icon are lined up
                    const int TitleYPadding = 3;
                    y += TitleYPadding;
                }
            ...
            if (showSubtitle)
            {
                float step = Styles.spaceBeforeSubtitle * compactY;
                headerHeight += step;
                y += step;
            ...
            if (showIcons)
            {
                ...
                for (int i = 0; i < description.icons.Length; i++)
                {
                    ...
                    if (++iconRow % Styles.iconsPerColumn == 0) { iconCol++; iconRow = 0; }
                }
            }

            if (showSettings && settings?.Count > 0)
            {
                headerHeight += Styles.spaceBeforeSettings * compactY;
                y += Styles.spaceBeforeSettings * compactY;
                int lastIndex = settings.Count - 1;
                for (int i = 0; i < settings.Count; i++)
                {
                    var setting = settings[i];
                    var rect = settingsPositions[setting];
                    float settingWidth = rect.width;
                    using (LudiqGUIUtility.currentInspectorWidth.Override(settingWidth))
                    {
                        float sHeight = LudiqGUI.GetInspectorHeight(null, setting, settingWidth, settingLabelsContents[setting] ?? GUIContent.none);
                        settingsPositions[setting] = new Rect(headerTextX, y, settingWidth, sHeight);
                        float spacing = (i == lastIndex) ? -Styles.spaceBetweenSettings : Styles.spaceBetweenSettings;
                        y += sHeight + spacing;
                        headerHeight += sHeight + spacing;
                    }
                }
            }
            ...
            if (showHeaderAddon)
            {
                ...
                headerAddonPosition = new Rect(iconPosition.xMax, y, headerAddonWidth, headerAddonHeight);
            ...
            if (showPorts)
            {
                bool hasValuePorts = _valueInputs.Count > 0 || _valueOutputs.Count > 0;

                rowDividerPositions.Clear();
                controlInputDividersX.Clear();
                controlOutputDividersX.Clear();
                ...
                float currentInputsY = y;
                float currentOutputsY = y;

                int maxRows = _valueInputs.Count > _valueOutputs.Count ? _valueInputs.Count : _valueOutputs.Count;

                if (hasValuePorts)
                {
                    rowDividerPositions.Add((currentInputsY, SeparatorPosition.Full));
                }

                for (int i = 0; i < maxRows; i++)
                {
                    if (i < _valueInputs.Count)
                    {
                        var widget = _valueInputs[i];
                        widget.y = currentInputsY + (rowHeight * 0.5f) - (widget.GetHeight() * 0.5f);
                    }

                    if (i < _valueOutputs.Count)
                    {
                        var widget = _valueOutputs[i];
                        widget.y = currentOutputsY + (rowHeight * 0.5f) - (widget.GetHeight() * 0.5f);
                    }

                    currentInputsY += rowHeight;
                    currentOutputsY += rowHeight;

                    if (i < maxRows - 1)
                    {
                        SeparatorPosition length = SeparatorPosition.Full;

                        bool nextHasInput = (i + 1) < _valueInputs.Count;
                        bool nextHasOutput = (i + 1) < _valueOutputs.Count;

                        bool currentHasInput = i < _valueInputs.Count;
                        bool currentHasOutput = i < _valueOutputs.Count;

                        bool isLeftActive = nextHasInput || currentHasInput;
                        bool isRightActive = nextHasOutput || currentHasOutput;

                        if (isLeftActive && !isRightActive)
                            length = SeparatorPosition.Left;
                        else if (!isLeftActive && isRightActive)
                            length = SeparatorPosition.Right;

                        rowDividerPositions.Add((currentInputsY, length));
                    }
                }

                float portsHeight = Mathf.Max(currentInputsY - portStartY, currentOutputsY - portStartY);
                y = portStartY + portsHeight;

                if (hasValuePorts)
                {
                    portsBackgroundHeight += portsHeight - 6;
                    innerHeight += portsHeight - 6;
                }

                const int ExtraSpaceBeforePorts = 4;

                if (_controlInputs.Count > 0)
                {
                    int portCount = _controlInputs.Count;
                    float controlY = edgeY - (Styles.spaceBeforePorts + ExtraSpaceBeforePorts) - Styles.spaceAfterControlInputs;

                    float totalSlotSpace = edgeWidth - Styles.spaceBeforePorts;
                    float slotWidth = totalSlotSpace / portCount;

                    for (int i = 0; i < portCount; i++)
                    {
                        var widget = _controlInputs[i];
                        ...
                        if (i < portCount - 1)
                        {
                            controlInputDividersX.Add(edgeX + (slotWidth * (i + 1)));
                        }
                    }
                }
                ...
                if (_controlOutputs.Count > 0)
                {
                    int portCount = _controlOutputs.Count;
                    float maxCoHeight = 0f;

                    for (int i = 0; i < _controlOutputs.Count; i++)
                    {
                        float h = _controlOutputs[i].GetHeight();
                        if (h > maxCoHeight) maxCoHeight = h;
                    }

                    controlOutputsHeight = maxCoHeight;

                    const int ControlOutputsPadding = 3;
                    float controlY = innerY + innerHeight + Styles.spaceBeforePorts +
                    ExtraSpaceBeforePorts + controlOutputsHeight + ControlOutputsPadding + Styles.spaceBeforeControlOutputs;
                    ...
                    for (int i = 0; i < portCount; i++)
                    {
                        ...
                        if (i < portCount - 1)
                        {
                            controlOutputDividersX.Add(edgeX + (slotWidth * (i + 1)));
                        }
                    }
                }
        ...
        protected override bool dim
        {
            get
            {
                ...
                if (BoltCore.Configuration.dimIncompatibleNodes && canvas.isCreatingConnection)
                {
                    ...
                    foreach (var p in unit.ports)
                    {
                        if (canvas.connectionSource == p || canvas.connectionSource.CanValidlyConnectTo(p))
                        {
                            canConnect = true;
                            break;
                        }
        ...
        protected static readonly Color portsSeparatorColor = new Color(0.15f, 0.15f, 0.15f, 1f);
        ...
        protected void DrawPortsBackground()
        {
            if (canvas.showRelations)
            {
                foreach (var relation in unit.relations)
                {
                    ...
                    var destinationWidget = Single(destinationPort);
                    ...
                    Handles.DrawBezier(
                        ...
                        startTangent,
                        endTangent,
                        ColorPalette.unityBackgroundDark,
                        null,
                        3f
                    );
                }

                static Vector2 PortDirection(IUnitPort port)
                {
                    return port switch
                    {
                        ControlOutput => Vector2.down,
                        ControlInput => Vector2.up,

                        ValueOutput => Vector2.left,
                        ValueInput => Vector2.right,
            ...
            {
                float edgeX = edgePosition.x;
                float fullWidth = edgePosition.width;
                float halfWidth = fullWidth * 0.5f;
                float midX = edgeX + halfWidth;

#if NEW_UNIT_STYLE
                float outerY = outerPosition.y + 2f;
                float innerY = innerPosition.yMax - 2f;
#else
                float outerY = outerPosition.y + 4f;
                float innerY = innerPosition.yMax - 5f;
#endif

                int rowCount = rowDividerPositions.Count;
                for (int i = 0; i < rowCount; i++)
                {
                    float lineY = rowDividerPositions[i].Item1;
                    SeparatorPosition length = rowDividerPositions[i].Item2;

                    float x = edgeX;
                    float w = fullWidth;

                    if (length == SeparatorPosition.Left)
                    {
                        w = halfWidth;
                    }
                    else if (length == SeparatorPosition.Right)
                    {
                        x = midX;
                        w = halfWidth;
                    }

                    EditorGUI.DrawRect(new Rect(x, lineY, w, 1f), portsSeparatorColor);
                }

                int inputCount = controlInputDividersX.Count;
                for (int i = 0; i < inputCount; i++)
                {
                    EditorGUI.DrawRect(new Rect(controlInputDividersX[i], outerY, 1f, 16f), portsSeparatorColor);
                }

                int outputCount = controlOutputDividersX.Count;
                for (int i = 0; i < outputCount; i++)
                {
                    EditorGUI.DrawRect(new Rect(controlOutputDividersX[i], innerY, 1f, 16f), portsSeparatorColor);
                }
            }
        }
        ...
        public static class Styles
        {
            ...
            public static readonly float spaceBeforePorts = 4;
#if NEW_UNIT_STYLE
            public static readonly float spaceBeforeControlOutputs = 6;

            public static readonly float spaceAfterControlInputs = 20;
#else
